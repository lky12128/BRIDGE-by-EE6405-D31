...
from BRIDGE import BRIDGE_Model
...
def evaluate(eval_model, data_source, eval_batch_size):
    # evaluation uses teacher forcing batches
    eval_model.eval()
    total_loss = 0.
    with torch.no_grad():
            ...
            for t in range(single_answer_length - 1):
                ...
                for i in range(beam_size):
                    ...
                    if int(current_tokens[-1].item()) == sep_idx:
                        ...
                        # expand the hypothesis with encoder feedback
...
if __name__ == "__main__":
    ...
    # seed configuration returns multi_gpu flag
    ...
    model = BRIDGE_Model(pretrained_encoder,
                         pretrained_config,
                         configs['cls_index'],
                         configs['sep_index'],
                         configs['pad_index'],
    ...
    if task == "Train":
        # apply weight decay only to non norm parameters
        ...
        for epoch in range(1, epochs + 1):
            ...
            for i, batch in enumerate(get_batch(running_data.train.examples, batch_size)):
                ...
                bsz = batch[0].size(0)
                if multi_gpu:
                    # average losses from each replica
                    lm_losses = sum([lm_loss.mean() for lm_loss in lm_losses])
                ...
                if i % log_interval == 0 and i > 0:
                    cur_loss_lm = total_loss / log_interval
